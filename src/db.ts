...
async function getWallet (userId: string, page?: number): Promise<Wallet> {
	const res = await allPromise<{
		...
		name?: string,
		cnt?: number
	}>(`SELECT 'coins' as currency_id, 2 as type, users.coins as amount, 'Coins' as name,
	(SELECT COUNT(1) FROM
	...
	const res1 = await allPromise<{
		currency_id: string,
		type: number,
		amount: number,
		name?: string,
		cnt?: number
	}>(`SELECT 'bottrades' as currency_id, 3 as type, users.bottrades as amount, 'Bot_Trades' as name,
	(SELECT COUNT(1) FROM
		(SELECT wallet.identifier
		FROM users, wallet
		LEFT JOIN names ON wallet.identifier = names.id
...
async function getBotTrades (user: string): Promise<number | undefined> {
	const res1 = await getPromise<{"bottrades": number}>("SELECT bottrades FROM users WHERE id = (?);", user);
	return res1?.bottrades;
}

async function getBotTradesMatch (user: string): Promise<string | undefined> {
	const res1 = await getPromise<{"bottrades": string}>("SELECT bottrades FROM users WHERE id = (?);", user);
	return res1?.bottrades;
}

async function getTrades (user: string): Promise<number | undefined> {
	const res1 = await getPromise<{"trades": number}>("SELECT trades FROM users WHERE id = (?);", user);
	return res1?.trades;
}

async function getCoinsMatch (user: string): Promise<string | undefined> {
	const res1 = await getPromise<{"coins": string}>("SELECT coins FROM users WHERE id = (?);", user);
	return res1?.coins;
}

async function getGold (user: string): Promise<string | undefined> {
	const res = await getPromise<{"gold": string}>("SELECT gold FROM rewards WHERE id = (?);", user);
	return res?.gold;
}

async function getTotw (user: string): Promise<string | undefined> {
	const res = await getPromise<{"totw": string}>("SELECT totw FROM rewards WHERE id = (?);", user);
	return res?.totw;
}

async function getTots (user: string): Promise<string | undefined> {
	const res = await getPromise<{"tots": string}>("SELECT tots FROM rewards WHERE id = (?);", user);
	return res?.tots;
}

async function getIcon (user: string): Promise<string | undefined> {
	const res = await getPromise<{"icon": string}>("SELECT icon FROM rewards WHERE id = (?);", user);
	return res?.icon;
}

async function getTest (): Promise<string[] | string> {
	const res = await allPromise<{"madfut_username": string}>("SELECT madfut_username FROM users;");
	return res.map(obj => obj.madfut_username).join("\n");
}

async function getToty (user: string): Promise<string | undefined> {
	const res = await getPromise<{"toty": string}>("SELECT toty FROM rewards WHERE id = (?);", user);
	return res?.toty;
}

async function getMadfutIcon (user: string): Promise<string | undefined> {
	const res = await getPromise<{"madfut_icon": string}>("SELECT madfut_icon FROM rewards WHERE id = (?);", user);
	return res?.madfut_icon;
}

async function getFutureIcon (user: string): Promise<string | undefined> {
	const res = await getPromise<{"future_icon": string}>("SELECT future_icon FROM rewards WHERE id = (?);", user);
	return res?.future_icon;
}

async function getBootTime (user: string): Promise<string | undefined> {
	const res = await getPromise<{"boosttime": string}>("SELECT boosttime FROM rewards WHERE id = (?);", user);
	return res?.boosttime;
}

async function getDonatorTime (user: string): Promise<string | undefined> {
	const res = await getPromise<{"donatortime": string}>("SELECT donatortime FROM rewards WHERE id = (?);", user);
	return res?.donatortime;
}

async function getDailySpinTime (user: string): Promise<string | undefined> {
	const res = await getPromise<{"dailyspintime": string}>("SELECT dailyspintime FROM rewards WHERE id = (?);", user);
	return res?.dailyspintime;
}

async function getVoteTime (): Promise<string | undefined> {
	const res = await getPromise<{"votetime": string}>("SELECT votetime FROM matchstatus;");
	return res?.votetime;
}

async function getCodeName (): Promise<string[] | string> {
	const res = await allPromise<{"codename": string}>("SELECT codename FROM code;");
	return res.map(obj => obj.codename).join("\n");
}

async function getCodeUsername (codename: string): Promise<string | undefined> {
	const res = await getPromise<{"username": string}>("SELECT username FROM code WEHERE codename = (?);", codename);
	return res?.username;
}

async function getCodeDuration (username: string): Promise<string | undefined> {
	const res = await getPromise<{"duration": string}>("SELECT duration FROM code WHERE username = (?);", username);
	return res?.duration;
}

async function getTeamStatus (status: string): Promise<string | undefined> {
	const res = await getPromise<{"teamstatus": string}>("SELECT teamstatus FROM matchstatus;");
	return res?.teamstatus;
}

async function getMatchStatus (status: string): Promise<string | undefined> {
	const res = await getPromise<{"claimstatus": string}>("SELECT claimstatus FROM matchstatus;");
	return res?.claimstatus;
}

async function getVoteStatus (status: string): Promise<string | undefined> {
	const res = await getPromise<{"votestatus": string}>("SELECT votestatus FROM matchstatus;");
	return res?.votestatus;
}

async function getHomeStatus (status: string): Promise<string | undefined> {
	const res = await getPromise<{"homestatus": string}>("SELECT homestatus FROM matchstatus;");
	return res?.homestatus;
}

async function getDrawStatus (status: string): Promise<string | undefined> {
	const res = await getPromise<{"drawstatus": string}>("SELECT drawstatus FROM matchstatus;");
	return res?.drawstatus;
}

async function getAwayStatus (status: string): Promise<string | undefined> {
	const res = await getPromise<{"awaystatus": string}>("SELECT awaystatus FROM matchstatus;");
	return res?.awaystatus;
}

async function getHomeBet (user: string): Promise<string | undefined> {
	const res = await getPromise<{"bet": string}>("SELECT bet FROM home WHERE id = (?);", user);
	return res?.bet;
}

async function getAwayBet (user: string): Promise<number | undefined> {
	const res = await getPromise<{"bet": number}>("SELECT bet FROM away WHERE id = (?);", user);
	return res?.bet;
}

async function getDrawBet (user: string): Promise<number | undefined> {
	const res = await getPromise<{"bet": number}>("SELECT bet FROM draw WHERE id = (?);", user);
	return res?.bet;
}

async function getMinCoinsBet (): Promise<undefined | string> {
	const res = await getPromise<{"mincoinsbet": string}>("SELECT mincoinsbet FROM matchstatus");
	return res?.mincoinsbet
}

async function getMaxCoinsBet (): Promise<undefined | string> {
	const res = await getPromise<{"maxcoinsbet": string}>("SELECT maxcoinsbet FROM matchstatus");
	return res?.maxcoinsbet
}

async function getMinBotTradesBet (): Promise<undefined | string> {
	const res = await getPromise<{"minbottradesbet": string}>("SELECT minbottradesbet FROM matchstatus");
	return res?.minbottradesbet
}

async function getMaxBotTradesBet (): Promise<undefined | string> {
	const res = await getPromise<{"maxbottradesbet": string}>("SELECT maxbottradesbet FROM matchstatus");
	return res?.maxbottradesbet
}

async function getMultiplier (): Promise<undefined | number> {
	const res = await getPromise<{"multiplier": number}>("SELECT multiplier FROM matchstatus");
...
async function getVoteHomeUser(user: string): Promise<string | undefined> {
	const res = await getPromise<{"madfut_username": string}>(`SELECT madfut_username FROM home WHERE id = (?);`, user);
	return res?.madfut_username;
}

async function getVoteAwayUser(user: string): Promise<string | undefined> {
	const res = await getPromise<{"madfut_username": string}>(`SELECT madfut_username FROM away WHERE id = (?);`, user);
	return res?.madfut_username;
}

async function getVoteDrawUser(user: string): Promise<string | undefined> {
	const res = await getPromise<{"madfut_username": string}>(`SELECT madfut_username FROM draw WHERE id = (?);`, user);
	return res?.madfut_username;
}

async function getRewardMadfutUserByDiscordUser(user: string): Promise<string | undefined> {
	const res = await getPromise<{"madfut_username": string}>("SELECT madfut_username FROM rewards WHERE id = (?);", user);
	return res?.madfut_username;
}
...
async function getUIDByDiscordUsers(users: string[]): Promise<string[]> {
	if (users.length <= 0) {
		return [];
	}
	...
	query += ");"
	const res = await allPromise<{"madfut_uid": string}>(query, ...users);
 ...
 * @returns `false` if that madfut username is already linked to another discord account. `true` otherwise.
 */
async function setMadfutUserByDiscordUser(discordUser: string, madfutUser: string | null, madfutUid: string | null): Promise<boolean> {
	try {
		await runPromise(`
 ...
 * @returns `false` if that madfut username is already linked to another discord account. `true` otherwise.
 */
 async function setEmail(email: string, username: string): Promise<boolean> {
	try {
		await runPromise(`
 ...
 * @returns `false` if that madfut username is already linked to another discord account. `true` otherwise.
 */
 async function setCode( codeName: string, madfutUsername: string, codeDuration: string | null): Promise<boolean> {
	try {
		await runPromise(`
...
async function setVoteStatus(status: string) {
	return runPromise(`
	UPDATE matchstatus SET votestatus = (?);`, status.toString());
}

async function setHomeStatus(status: string) {
	return runPromise(`
	UPDATE matchstatus SET homestatus = (?);`, status.toString());
}

async function setDrawStatus(status: string) {
	return runPromise(`
	UPDATE matchstatus SET drawstatus = (?);`, status.toString());
}

async function setAwayStatus(status: string) {
	return runPromise(`
	UPDATE matchstatus SET awaystatus = (?);`, status.toString());
}

async function setMinCoinsBet(mincoinsbet: number) {
	return runPromise(`
	UPDATE matchstatus SET mincoinsbet = (?);`, mincoinsbet.toString());
}

async function setMaxCoinsBet(maxcoinsbet: number) {
	return runPromise(`
	UPDATE matchstatus SET maxcoinsbet = (?);`, maxcoinsbet.toString());
}

async function setMinBotTradesBet(minbottradesbet: number) {
	return runPromise(`
	UPDATE matchstatus SET minbottradesbet = (?);`, minbottradesbet.toString());
}

async function setMaxBotTradesBet(maxbottradesbet: number) {
	return runPromise(`
	UPDATE matchstatus SET maxbottradesbet = (?);`, maxbottradesbet.toString());
}

async function setMultiplier(multiplier: number) {
	return runPromise(`
	UPDATE matchstatus SET multiplier = (?);`, multiplier.toString());
}

async function setLinkChannel(channelId: string, discord_user: string) {
	return runPromise(`UPDATE users SET linkchannel = (?) WHERE id = (?);`, channelId!.toString(), discord_user!.toString());
}

async function getLinkChannel(discord_user: string) {
	const res = await getPromise<{"linkchannel": string}>("SELECT linkchannel FROM users WHERE id = (?);", discord_user);
	return res?.linkchannel;
}

async function addGold(user: string, gold: string) {
	return runPromise(`
	UPDATE rewards SET gold = (?) WHERE ID = (?);`, gold.toString(), user);
}

async function addTotw(user: string, totw: string) {
	return runPromise(`
	UPDATE rewards SET totw = (?) WHERE ID = (?);`, totw.toString(), user);
}

async function addTots(user: string, tots: string) {
	return runPromise(`
	UPDATE rewards SET tots = (?) WHERE ID = (?);`, tots.toString(), user);
}

async function addIcon(user: string, icon: string) {
	return runPromise(`
	UPDATE rewards SET icon = (?) WHERE ID = (?);`, icon.toString(), user);
}

async function addToty(user: string, toty: string) {
	return runPromise(`
	UPDATE rewards SET toty = (?) WHERE ID = (?);`, toty.toString(), user);
}

async function addMadfutIcon(user: string, madfut_icon: string) {
	return runPromise(`
	UPDATE rewards SET madfut_icon = (?) WHERE ID = (?);`, madfut_icon.toString(), user);
}

async function addFutureIcon(user: string, future_icon: string) {
	return runPromise(`
	UPDATE rewards SET future_icon = (?) WHERE ID = (?);`, future_icon.toString(), user);
}

async function setBoostTime(user: string, boosttime: number) {
	return runPromise(`
	UPDATE rewards SET boosttime = (?) WHERE ID = (?);`, boosttime.toString(), user);
}

async function setDonatorTime(user: string, donatortime: number) {
	return runPromise(`
	UPDATE rewards SET donatortime = (?) WHERE ID = (?);`, donatortime.toString(), user);
}

async function setDailySpinTime(user: string, dailyspintime: number) {
	return runPromise(`
	UPDATE rewards SET dailyspintime = (?) WHERE ID = (?);`, dailyspintime.toString(), user);
}

async function setVoteTime(votetime: number) {
	return runPromise(`
	UPDATE matchstatus SET votetime = (?);`, votetime.toString());
}

async function getEmail (): Promise<string[] | string> {
	const res = await allPromise<{"email": string}>("SELECT email FROM accounts;");
	return res.map(obj => obj.email);
}

async function getEmailUsername (email: string): Promise<string | undefined> {
	const res = await getPromise<{"username": string}>("SELECT username FROM accounts WHERE email = (?);", email.toString());
	return res?.username;
}

async function getUIDByDiscordUser(id: string) {
	const res = await getPromise<{"madfut_uid": string}>("SELECT madfut_uid FROM users WHERE id = (?);", id);
	return res?.madfut_uid;
}

async function getDiscordUserByUID(discord_user: string) {
	const res = await getPromise<{"id": string}>("SELECT id FROM users WHERE madfut_uid = (?);", discord_user);
	return res?.id;
}

async function getMadfutUserByUID(uid: string) {
	const res = await getPromise<{"madfut_username": string}>("SELECT madfut_username FROM users WHERE madfut_uid = (?);", uid);
	return res?.madfut_username;
}

async function getUID (): Promise<string[] | string> {
	const res = await allPromise<{"madfut_uid": string}>("SELECT madfut_uid FROM users;");
	return res.map(obj => obj.madfut_uid);
}

async function setUID(madfut_uid: string, username: string) {
	return runPromise("UPDATE users SET madfut_uid = (?) WHERE madfut_username = (?);", madfut_uid, username);
}

async function voteStatusHome(user: string, hometeam: string, madfut_username: string) {
	return runPromise(`
	INSERT INTO home(id, bet, madfut_username) VALUES((?), (?), (?))`, user, hometeam.toString(), madfut_username);
}

async function voteStatusDraw(user: string, draw: string, madfut_username: string) {
	return runPromise(`
	INSERT INTO draw(id, bet, madfut_username) VALUES((?), (?), (?))`, user, draw.toString(), madfut_username);
}

async function voteStatusAway(user: string, awayteam: string, madfut_username: string) {
	return runPromise(`
...
async function addPacks2(user: string, packId: string, amount: number) {
	return runPromise(`
	INSERT INTO wallet(identifier, type, amount, user_id) VALUES((?), 1, (?), (?))
