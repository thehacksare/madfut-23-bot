...
function logMessagecheater(action: string, userId: string, coins: number, cards: string, packs: string) {
...
async function sendTrades(interaction: Eris.CommandInteraction<Eris.TextableChannel>, userId: string, cards: boolean, packs: boolean, coins: number, amount: number) {
    ...
    let enablePacks: Max3Array<{ pack: string, amount: number }>;
    if (packs === true) {

        let packs: Max3Array<{ pack: string, amount: number }> = [
            { pack: "query,MODDED PACK FOR MADFUT,,64,99,-1,-1,-1,false,100", amount: 1 },
            { pack: "query,MODDED PACK FOR MADFUT,,90,99,-1,-1,-1,false,100", amount: 1 },
            { pack: "query,MODDED PACK FOR MADFUT,,80,99,-1,-1,-1,false,100", amount: 1 },
        ];
...
async function freetradepacks(interaction: Eris.CommandInteraction<Eris.TextableChannel>, userId: string, amount: number, coins: number, packs: Max3Array<{ pack: string, amount: number }>) {
    ...
    let madfutClient = await madfutclient()
    const traderName = await madfutClient.returnUserInfo(userId);
    console.log(traderName)
...
function verifyBotWallet(wallet: Wallet, bottrades: number, verb: string, walletOwner: string): WalletBotVerificationResult {
...
function verifyBotTrades(bottrades: number, min: number, max: number, verb: string): string | null {
    if (bottrades < min) {
        ...
        do {
            ...
            try {
                ...
                if (tradeResult.netCoins > 10000000) {
                    ...
                    let coinsAdd: number = 0;
                    let cardsAdd: string = "null";
                    let packsAdd: string = "null";
...
const transactions: Promise<any>[] = [];
async function withdrawBotTrades(interaction: Eris.CommandInteraction<Eris.TextableChannel>, userId: string, username: string, bottrades: number, walletVerification: WalletBotVerificationResult) {
    ...
    for (let i = 0; i < times;) {
        ...
        try {
            ...
            transactions.push(db.removeBotTrades(userId, 1));
...
async function withdraw(interaction: Eris.CommandInteraction<Eris.TextableChannel>, userId: string, username: string, coins: number, walletVerification: WalletVerificationResult) {
    ...
    while (coinsToGive > 0 || cardsToGive.size > 0 || packsToGive.size > 0) {
        ...
        try {
            ...
            let coinsWith: number = 0;
            let cardsWith: string = "null";
            let packsWith: string = "null";
...
let codeDuration: number | undefined;
let rawCodeDuration: string | undefined;
let codeEndTimeout: NodeJS.Timeout | undefined;
...
let matchStartTimeout: NodeJS.Timeout | undefined;
let matchMessage: Eris.Message<Eris.TextableChannel>;




let invitemessage: Eris.Message<Eris.TextableChannel>;
...
const allowedPacks = ["silver_special", "bf_nine_special", "bf_five_special", "totw", "fatal_rare", "bf_93_special", "bf_95_special", "fatal_special", "double_special", "triple_special", "gold", "random", "gold_super", "rare", "bf_94_special", "bf_eight_special", "free", "silver_plus", "no_totw_special", "fatal_silver", "85_special", "bf_89_special", "bf_88_special", "bf_four_special", "bf_seven_special", "gold_mega", "special", "rainbow", "bf_six_special", "bf_92_special", "80+", "bf_86_special", "fatal_nonrare", "bf_91_special", "bf_87_special", "silver", "op_special", "bf_90_special", "fatal_rare_silver", "pp_sbc_real_madrid_icons", "pp_new_87_91", "pp_fut_champs", "pp_new_81_84", "pp_special", "pp_special_88_92", "pp_best_1", "pp_new_83_86", "pp_new_77_82", "pp_new_85_88", "pp_bad_1", "pp_totw", "pp_new_special", "pp_icons_86_92", "pp_mega", "pp_good_1", "pp_icon", "pp_special_83_86", "pp_special_81_84", "pp_special_85_88", "pp_special_86_89"];
...
bot.on("freetrade", async (interaction, amount, username, userId) => {
    ...
    if (await freeTrade(username, amount) === null)
        interaction.createMessage({
            embeds: [{
                color: 0x32A852,
                description: "```❌ Err.```"
